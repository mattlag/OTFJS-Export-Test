...
Path.prototype.fromSVG = function (pathData, options = {}) {
	...
	for (let i = 0; i < pathData.length; i++) {
		...
		if (number.indexOf(token) > -1) {
			...
			throw new Error(
				'Unsupported path command: ' + token + '. Currently supported commands are ' + supportedCommands.split('').join(', ') + '.'
			);
...
Path.prototype.toPathData = function (options) {
	...
	for (let i = 0; i < commandsCopy.length; i += 1) {
		...
		if (cmd.type === 'M') {
			...
			d += 'Q' + packValues(cmd.x1, flipY ? flipYBase - cmd.y1 : cmd.y1, cmd.x, flipY ? flipYBase - cmd.y : cmd.y);
		} else if (cmd.type === 'Z') {
			d += 'Z';
		}
	}
...
Parser.prototype.parseTupleVariationStoreList = function (axisCount, flavor, glyphs) {
	...
	for (let i = 0; i < glyphCount; i++) {
		...
		glyphVariations[i] = length
			? this.parseTupleVariationStore(glyphVariationDataArrayOffset + currentOffset, axisCount, flavor, glyphs, i)
			: void 0;
...
Parser.prototype.parseTupleVariationStore = function (tableOffset, axisCount, flavor, glyphs, glyphIndex) {
	...
	if (this.relativeOffset !== tableOffset + dataOffset) {
		console.warn(
			`Unexpected offset after parsing tuple variation headers! Expected ${tableOffset + dataOffset}, actually ${this.relativeOffset}`
		);
...
function makeTopDictIndex(topDict) {
	const t = new table_default.Record('Top DICT INDEX', [{ name: 'topDicts', type: 'INDEX', value: [] }]);
	t.topDicts = [{ name: 'topDict_0', type: 'TABLE', value: topDict }];
	return t;
}
function makeStringIndex(strings) {
	const t = new table_default.Record('String INDEX', [{ name: 'strings', type: 'INDEX', value: [] }]);
	t.strings = [];
...
function makeGlobalSubrIndex() {
	return new table_default.Record('Global Subr INDEX', [{ name: 'subrs', type: 'INDEX', value: [] }]);
}
...
function makeCharStringsIndex(glyphs, version) {
	const t = new table_default.Record('CharStrings INDEX', [{ name: 'charStrings', type: 'INDEX', value: [] }]);
	for (let i = 0; i < glyphs.length; i += 1) {
		const glyph = glyphs.get(i);
...
function makeCFFTable(glyphs, options) {
	...
	const startOffset =
		t.header.sizeOf() + t.nameIndex.sizeOf() + t.topDictIndex.sizeOf() + t.stringIndex.sizeOf() + t.globalSubrIndex.sizeOf();
					...
					table_default.tableList('ligature', ligatureSet, function (ligature) {
						return new table_default.Table(
							...
							[{ name: 'ligGlyph', type: 'USHORT', value: ligature.ligGlyph }].concat(
								table_default.ushortList('component', ligature.components, ligature.components.length + 1)
							)
						);
					})
				);
					...
					return new table_default.Table(
						...
						table_default.tableList('sequenceRule', sequenceRuleSet, function (sequenceRule) {
							let tableData = table_default
								...
								.concat(table_default.ushortList('inputSequence', sequenceRule.input, sequenceRule.input.length + 1));
					...
					return new table_default.Table(
						...
						table_default.tableList('classSeqRule', classSeqRuleSet, function (classSeqRule) {
							let tableData = table_default
								...
								.concat(table_default.ushortList('seqLookupCount', [], classSeqRule.lookupRecords.length));
					...
					return new table_default.Table(
						...
						table_default.tableList('chainRule', chainRuleSet, function (chainRule) {
							let tableData = table_default
								...
								.concat(table_default.ushortList('inputGlyph', chainRule.input, chainRule.input.length + 1))
								.concat(table_default.ushortList('lookaheadGlyph', chainRule.lookahead, chainRule.lookahead.length))
								.concat(table_default.ushortList('substitution', [], chainRule.lookupRecords.length));
...
function makeFvarTable(fvar, names) {
	...
	for (let j = 0; j < fvar.instances.length; j++) {
		result.fields = result.fields.concat(makeFvarInstance(j, fvar.instances[j], fvar.axes, optionalFields));
	}
	return result;
}
...
function fontToSfntTable(font) {
	...
	if (!names.macintosh.preferredSubfamily) {
		names.macintosh.preferredSubfamily =
			fontNamesMacintosh.fontSubfamily || fontNamesUnicode.fontSubfamily || fontNamesWindows.fontSubfamily;
	}
...
var decodeSvgDocument =
	typeof DecompressionStream === 'function' ? decodeSvgDocumentWithDecompressionStream : decodeSvgDocumentWithTinyInflate;
			...
			for (let i = 0; i < this.avar().axisSegmentMaps.length; i++) {
				...
				for (let j = 0; j < segment.axisValueMaps.length; j++) {
					...
					if (j >= 1 && normalized[i] < pair.fromCoordinate) {
						...
						normalized[i] =
							((normalized[i] - prev.fromCoordinate) * (pair.toCoordinate - prev.toCoordinate) + Number.EPSILON) /
	...
	getTransform(glyph, coords) {
		...
		if (this.font.tables.hvar) {
			...
			glyph.advanceWidth = transformedGlyph.advanceWidth = Math.round(
				glyph._advanceWidth + this.getVariableAdjustment(transformedGlyph.index, 'hvar', 'advanceWidth', coords)
			);
			...
			glyph.leftSideBearing = transformedGlyph.leftSideBearing = Math.round(
				glyph._leftSideBearing + this.getVariableAdjustment(transformedGlyph.index, 'hvar', 'lsb', coords)
			);
		}
...
FeatureQuery.prototype.lookupFeature = function (query) {
	...
	if (!feature)
		return new Error(
			`font '${(this.font.names.unicode || this.font.names.windows || this.font.names.macintosh).fullName.en}' doesn't support feature '${
				query.tag
